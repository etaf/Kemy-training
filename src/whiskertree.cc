...
KemyBuffers::WhiskerTree WhiskerTree::DNA( void ) const
{
  KemyBuffers::WhiskerTree ret;
  ...
  if ( is_leaf() ) {
    ...
    for ( auto &x : _children ) {
      KemyBuffers::WhiskerTree *child = ret.add_children();
      *child = x.DNA();
    }
  }
