...
Whisker::Whisker( const unsigned int s_window_increment, const double s_window_multiple,  const MemoryRange & s_domain )
  : _generation( 0 ),
...
{
  char tmp[ 256 ];
  snprintf( tmp, 256, "{%s} gen=%u usage=%.4f => (win=%d + %f * win)",
	    _domain.str().c_str(), _generation, double( _domain.count() ) / double( total ), _window_increment, _window_multiple);
  return tmp;
}
